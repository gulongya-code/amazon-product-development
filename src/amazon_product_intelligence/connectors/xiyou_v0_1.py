...
XIYOU_CAPABILITIES = (
    ...
    _capability(
        "product.parent_asin",
        CapabilityStatus.AVAILABLE,
        source_field="parentAsin",
        operation="asin_variations",
        payload_kind="asin_variations_http_v2",
        kind=ObservationKind.PRODUCT_FACT,
    ...
    _capability(
        "metric.bsr",
        CapabilityStatus.AVAILABLE,
        source_field="trends[].values[].rank",
        operation="asin_bsr_trends",
        payload_kind="asin_bsr_trends_http_v2",
        kind=ObservationKind.METRIC,
        names=("bsr",),
    ),
    _capability(
        "metric.bsr_context",
        CapabilityStatus.AVAILABLE,
        source_field="categoryTree[]",
        operation="asin_bsr_trends",
        payload_kind="asin_bsr_trends_http_v2",
        notes="Context-only capability carried on metric.bsr rank_context; it is not a second value observation.",
    ),
    _capability(
