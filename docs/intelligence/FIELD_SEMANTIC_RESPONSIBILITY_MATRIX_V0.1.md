...
| F100 | `workbook.product_structure.minimum_comparable_price` | Minimum Comparable Price | CALCULATED | DEFINED_READY | AGGREGATION | HIGH | CALC,PI,WB | Governed Comparable Product Set `COMPARABLE` members plus compatible resolved prices | Calculation Engine, downstream of Comparable Product Set Contract V0.1 | F=N;AI=N;M=N;P=C;A=Y;B=Y | BUSINESS_RULE_BLOCKED | P0_MEMBERSHIP_SOURCE | `NO — BLOCKED BY MEMBERSHIP SOURCE`; candidate inventory and current Workbook projection are not the governed set. |
| F101 | `workbook.product_structure.maximum_comparable_price` | Maximum Comparable Price | CALCULATED | DEFINED_READY | AGGREGATION | HIGH | CALC,PI,WB | Governed Comparable Product Set `COMPARABLE` members plus compatible resolved prices | Calculation Engine, downstream of Comparable Product Set Contract V0.1 | F=N;AI=N;M=N;P=C;A=Y;B=Y | BUSINESS_RULE_BLOCKED | P0_MEMBERSHIP_SOURCE | `NO — BLOCKED BY MEMBERSHIP SOURCE`; same governed peer set and dependency direction as minimum. |
